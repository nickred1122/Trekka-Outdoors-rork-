...
///
/// This file is deliberately duplicated, byte for byte, from the app target. An
/// extension is a separate process with its own binary, so a type crossing
/// between them has to exist in both — and the encoded shape is the contract, so
/// the two copies must never be allowed to drift.
///
