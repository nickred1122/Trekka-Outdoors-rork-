...
nonisolated struct StrengthSet: Identifiable, Sendable, Equatable {
    var id: UUID = UUID()
    var exercise: String
    var reps: Int
...
/// The movements offered on the wrist.
///
/// Deliberately a curated list rather than an exhaustive database: scrolling a
/// thousand exercises on a watch between sets is worse than typing, and the
/// long tail belongs on the phone. Anything missing can still be logged under
/// the closest match.
nonisolated enum GymExerciseLibrary {
    /// Body mass used to score bodyweight work, in kilograms. Mirrored from the
    /// phone's latest Health reading, the same way the heart-rate ceiling and
    /// unit choices are mirrored into their static holders.
