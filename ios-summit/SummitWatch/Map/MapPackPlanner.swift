...
nonisolated enum MapPackPlanner {
    ...
    /// The ceiling for a download the watch runs itself.
    ///
    /// Far tighter than the phone's, for reasons that are physical rather than
    /// cautious: the wrist fetches over a slower radio, has a fraction of the
    /// storage, and is holding the screen awake for the whole download. A pack
    /// that takes the phone a minute would take the watch the better part of
    /// twenty, and an athlete is not going to stand still for that.
    static let watchCeiling = 850
    ...
    static func plan(
        ...
        return clamped(Plan(vector: vector, terrain: terrain), ceiling: ceiling)
    }
    ...
    /// shape of the ground.
    private static func clamped(_ plan: Plan, ceiling: Int) -> Plan {
